...
function formatCurrency(amount: number, symbol: string) {
  ...
  return `${operatorCurrency || symbol}${amount.toFixed(2)}`;
}
...
export default function RideSuccessfulPage() {
  ...
  const operatorCurrency = useOperatorParamsStore.getState().data?.user_web_config?.currency ||
    useOperatorParamsStore.getState().data?.user_web_config?.currency_symbol ||
    '₹';
  const currencySymbol = operatorCurrency || region?.region_fare?.currency_symbol;
  ...
  const handleBackToHome = () => {
    resetBooking();
    navigateWithLoader(router, `/${locale}/home`);
  };
        ...
        <Card className="mt-6 rounded-3xl border-0 shadow-sm p-6 sm:p-8">
          ...
          <div className="grid grid-cols-1 gap-6">
            ...
            <SelectedItems
              ...
              priceComponent={
                <PriceBlock
                  currencySymbol={operatorCurrency || region.region_fare?.currency_symbol || "₹"}
            ...
            {appliedCouponData && (
              <div className="flex justify-between items-center py-2 border-b border-gray-100">
                <span className="text-base font-medium text-gray-700">Coupon Applied</span>
                <span className="text-base font-semibold text-green-600">
                  {appliedCouponData.title}
                </span>
              </div>
            )}
